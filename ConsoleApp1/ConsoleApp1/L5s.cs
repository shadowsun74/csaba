...
// -----------------------
// LESZ MAJD KÉSŐBB: referencia, egyik script odaszól a másiknak -- majd tanuljuk
...
// vágósik, elülső, hátulsó vágósík térből ezt vágjuk ki. Ez kb. a kamera mit lát, tól-ig
//-----------------

// Sprite

// ahhoz, hogy hazsnáljuk, minimum kell előtte a package Manager -- utána 2d sprite (install)
// ugyanitt van pl. a sprite animációhoz a rigging és mozgatás is (bone elemek. A sprite-hez csontszerű elemeket rendelünk, kb: fej, nyak,
...
// sprite sheet - megtekinthető, ha betöltöttük a sprite-ot (sprite editor a menüben

// -----------------------

// Sprite Editor most már elérhető ha UNITY-ben telepítettük a fent leírtakat
...
// Sprite renderer vs Sprite renderer - sprite-nél mindíg egyik sprite vagy előrébb, vagy hétrébb, itt nincs átfedés (félig elül, félig hátul
// 3d esetén lehet átfedés
// Ha mindkettőt nullába tesszük, akkor nem tudjuk megmondani, melyik lesz elől, málység vátoztatás kell, a gép dönti el ugyanazon a pozíción

// Sorting LAyer, sorrendet ad a spriteoknak.

// kb a 2D-ről a tanár ennyit akart elmondani, végignéztük a menü egy részét, a többit majd ki kell találni

//--------------------------------



// --------------------------------------- A FÉNY "DIMMELÉSE" (GYENGE-ERŐS) --------------------------------------
...
class LightDimmer : MonoBehaviour
{
 ...
 // [SerializeField] Color c1, c2;                  // Menüben a szín, aminek két eleme a c1 és c2. Ezzel "csak" létrehozom
    ...
    [SerializeField] Color c1 = Color.white;        // külön-külön is létrehozható, ezzel egyből színt is tudok naki adni
...
class LightDimmer : MonoBehaviour
{
    ...
    [SerializeField, Range(0,1) ] float dim = 0; // plusz ezzel a sorral azt csináltuk, hogy csúszkaként hozom létre. MINDÍG 0-1 közütti érték!
    	...
    	void OnValidate()               // ezzel oldjuk meeg, hogy bármilyen változásnál unity-n belül is egyből lássuk a változtatást
...
//  [SerializeField, Range(0, 1)] float dim = 0         // kidobtuk ezt a sort. Nem kézzel akarjuk kezelni, hanem "simítva" menjen oda-vissza
    ...
    void Update()
    {
        ...
        float sin = Mathf.Sin(t);                       // a színusz változőnak a Mathf-ből a szinuszt használjuk (t változó értékkel)           
        ...
        sin += 1;                                       // ezzel a -1 ből pl nulla lesz, -0,5 -ből 1,5... és így tovább
        sin = sin / 2;                                  //ezzel meg elosztjuk kettővel. Így az érték biztosan nem megy 1 fölé
...
class LightDimmer : MonoBehaviour
{
    ...
    [SerializeField] Color c1 = Color.white;
    [SerializeField] Color c2 = Color.white;
    [SerializeField] float frequency = 1;           // legyen sebességes is, milyen gyorsan ér át egyik színből a másikba és vissza
    ...
    {
        float t = Time.time;   
        t *= frequency;                             // az előzőekben lekérdezett, játékben eltelt időt szorozzuk a frekvenciával
        t *= Mathf.PI * 2;                          // azzal kell szorozni, ami a teljes szélesség (skála), 2*PI radián, mert fokkban kell

        float sin = Mathf.Sin(t);
        sin += 1;                                   // ezeket átemeltük a LERP elé, így működik jól
        sin /= 2;
...
// UNITY: collidereket levesszük az enemy-ről, gyerek objectnél, játékosról is. A lényeg, hogy felesleges ennyi, hacsak pl. nem sebzést
// akarnál nézni, külön pl: headshot, akkor kell külön, de egyszerűbb játékoknál az "egész"-re nézzük
...
class Damager : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)                 // "OnTriggerEnter" új. Sok ilyen paramétere van még. 2d-s végű = 2d játékban !
...
// amikor a 2 collider találkozik, ellenőrzi a scipteket, megnézi a SW minek kell történnie.
...
// Ha menetközben átkattintok pl Projekt Asset-ekről, Consolee-re, és nem kattintok a scene-be, akkor play után pl. a karakterem nem megy.
...
class HealthObject : MonoBehaviour
{
    ...
    {                                 // a PUBLIC mondja meg, hogy a script nem csak önmagában dolgozik, scriptek között így tudunk átadni
                                      // az INT.DAMAGE ponttal volt írva, de nem biztos, hogy úgy kell, kijavítottam, de ha nem működik,
                                      // majd írjuk vissza
...
class Damager : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)  
    {
        ...
        HealthObject ho = go.GetComponent<HealthObject>(); // ho - mint healthobject változó. Kacsacsőr = ÚJ. Ez is zárójel, spéci, mezei
                                                           // metódus. Nincs paramétere, csak paraméterszerűsége, nem lehet bárhol használni,
                                                           // csak aki kezeli, ez itt a a kacsacsőrben az OZTÁLYNÉV a másik UNITY scriptből
                                                           // Ez generikus függvény (olvass utána ha akarsz)

        if(ho != null)                                     // null egy érték
        {
            ho.damage(damage);                             // UNITY SCRIPT MŰKÖDÉS: ha itt rányomok az F12.re, akkor innen oda ugrik a másik
                                                           // kódra, ahol használom (ha nyitva van több kód is
        }                                                  // ha a helath object nem nulla, akkor a health object sérülése a damage-vel egyenlő
    }
}
...
class HealthObject : MonoBehaviour
{
    ...
    public int currentHealth;                           // ez itt az új- Publikusan hasznéljuk a következőkben majd a currentHealth-et
    ...
    public void Damage(int damage)
    {
       //  currentHealth -= damage;
        ...
        if (currentHealth == 0) return;                 // PROGRAOZÁSNÁL JAVSOLT: először a "nem megyünk tovább" minden lehetséges esetre.
...
// ez előtt tettük publikussá a "public int currentHealth-ot (a fenti HealthObject scriptben)
...
public class Mover : MonoBehaviour
{
    ...
    {                                                                   // valószínűbb, hogy csak nem kell a "private" elé
        ...
        HealthObject healthObject = GetComponent<healthObject>();       // ha nulla, akkor a HealthObject változója (healtObject) kapja meg a 
                                                                        ...
                                                                        // legyen neki mefelelő értéke, amikor indítjuk először és még nincs
    void Update()
    {
...
public class Mover : MonoBehaviour
{
    ...
    [SerializeField] bool moveInCameraSpace = true;
    [SerializeField] float angularVelocity = 100;
    [SerializeField] HealthObject healthObject;                             // HO = hO: egyenlőség hiba volt, ki
    ...
    void Update()
    {
        //  if (healthObject.currentHealth <= 0) return;            // ezt már előbb kidobtuk, az eddigi feladatok, lépések miatt nem kell
...
// HEALTH OBJECT HIBA VOLT JAVÍTVA ITT - most ez vagy az előzőre vonatkozik az "=" problémára vagy nem tudom, mivel az előző HealthObject
...
class HealthObject : MonoBehaviour
{
    ...
    public bool IsAlive()                                   // ez változott még eggyel ez előtt: felvettük, vizsgáljuk, hogy a karakter él-e
    {
...
public class Mover : MonoBehaviour
{
    ...
    [SerializeField] bool moveInCameraSpace = true;
    [SerializeField] float angularVelocity = 100;
    [SerializeField] HealthObject healthObject;                 // egyenlő jel ki
...
public class Collector : MonoBehaviour                              // Ez  ascript a gyűjtéshez kell
{
    [SerializeField] int collected = 0;                             // számoló, nézi, mennyi érmét gyűjtöttünk
    ...
    {
        Collectable c = other.GetComponent<Collectable>();          // a "c" változó értéke az other (összes) komponensből csak a "Collectable"
                                                                    // Class-t nézze. Ezzel azt is megoldjuk, nehogy "begyűjtsön" mást
        collected++;                                                // egyszerűen növeljük az értékét eggyel "ütközéskor"
    }
}
...
public class Collectable : MonoBehaviour                            // Ez a script a gyűjthető cuccok miatt kell
{
...
public class Collector : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (c != null)                                              // miután fent már publikusnak tettük a COllectible alatt a "GetValue()"-t
            collected += c.GetValue();                              // itt a "collected++" helyett - de csak ha a "C" nem egyenlő nullával
...
// FONTOS: A "RANDOM" létezik a "USING system"-ben is (ez az, amit általában az első pár órán UNITY nélkül használtunk) illetve a
// using UnityEngine-ben is van, de ez már felparaméterezett, és máshogy működik. Figyelni kell, hogy a kódban nehogy pl. belekeveredjen
